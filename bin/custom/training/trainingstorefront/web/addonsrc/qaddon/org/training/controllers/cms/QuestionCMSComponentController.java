...
public class QuestionCMSComponentController extends AbstractCMSAddOnComponentController<QuestionsCMSComponentModel> {
    ...
    public QuestionCMSComponentController(QuestionsProductFacade questionsProductFacade) {
    ...
    protected void fillModel(
        ...
        if (productData != null && productData.getCode() != null) {
            ...
            if (CollectionUtils.isNotEmpty(productData.getQuestions())) {
                questions = productData.getQuestions();
            } else {
                questions = questionsProductFacade.getQuestionsForProduct(productData.getCode());
            }

            model.addAttribute(QUESTION_ATTRIBUTE_NAME,
                    questions.stream().limit(numberOfQuestionsToShow).toList());
        }
